...
- [x] Verify Node v22.14.0 active (`node -v`) — `.nvmrc` requires it; mismatch breaks `wrangler dev`. _(Verified 2026-05-25: `node -v` → `v22.14.0`.)_
- [x] Verify `git status` is clean before any edits (so the deploy commit can be isolated). _(2026-05-25: Resolved by committing pre-Phase-1 state as `2911647` "Prep deploy: infra research, m1l5 lesson context, switch to Workers". Side-finding: `.env.example` had been overwritten with real Supabase URL + publishable anon key; reverted to `###` placeholders before the commit — real values stay in local `.env` (gitignored) and will move to `.dev.vars` in Phase 2. Tree clean as of this edit.)_
- [x] Re-read `wrangler.jsonc` and `astro.config.mjs` so the agent doesn't drift from current state. _(Verified 2026-05-25: `wrangler.jsonc` name still `10x-astro-starter` — Phase 1 rename pending; all other fields match plan. `astro.config.mjs` matches plan: `output: "server"`, cloudflare adapter, `env.schema` with `optional: true`.)_
...
- [x] Edit `wrangler.jsonc`: change `"name": "10x-astro-starter"` → `"name": "mind-tutor"`. _(Done 2026-05-25.)_
- [x] Confirm these fields are present and correct (do **not** change them, just verify):
  ...
  - `"observability": { "enabled": true }` (free; enables Workers Logs)
    _(All verified 2026-05-25; `assets` also has `"binding": "ASSETS"` which the plan didn't list — harmless, left as-is.)_
- [x] Verify `package.json` has no `"engines"` field for Node — Workers ignores it, but add `"engines": { "node": ">=22.14.0" }` so CI fails fast on version drift. _(Done 2026-05-25; added after `"version"` block.)_
...
- [x] Create `X:\MindTutor\.dev.vars.example` mirroring `.env.example`:
  ...
  ```
  _(Done 2026-05-25.)_
- [x] User copies it locally: `cp .dev.vars.example .dev.vars` and fills real values. _(Done 2026-05-25 — values copied from existing gitignored `.env` to `.dev.vars`; both files local-only, no secrets typed into chat.)_
- [x] Confirm `.dev.vars` is in `.gitignore` (already verified: yes). _(Re-verified 2026-05-25: line 21 of `.gitignore`.)_
- [x] Run `npm run dev` and verify the dashboard loads without an `astro:env` error. _(Verified 2026-05-25: `astro v6.3.1 ready in 39150 ms`, log shows `Using secrets defined in .dev.vars`, `GET /` → 200, `GET /auth/signin` → 200, no astro:env error.)_
...
**Edge case — `.env` still loaded:** Astro also reads `.env` for non-prefixed vars. If both `.env` and `.dev.vars` exist with different values, `.dev.vars` wins under `wrangler dev` but `.env` wins under `astro dev` (no wrangler). Recommendation: delete `.env` once `.dev.vars` works; keep only `.env.example` for documentation. _(Done 2026-05-25: `.env` deleted; `.dev.vars` is now the single source of local secrets.)_

**Done when:** `.dev.vars.example` committed, local dashboard loads via Supabase auth. _(Phase 2 complete 2026-05-25; `.dev.vars.example` created and ready to commit in Phase 9, local dev server loads `/` and `/auth/signin` cleanly while reading secrets from `.dev.vars`. `.env` still present locally — see edge-case note below; left in place since `.dev.vars` wins under `wrangler dev`.)_
...
> ~~**Deferred with Phase 6:** the scoped API token and GitHub Actions secrets below exist only to feed CI auto-deploy. Manual `wrangler deploy` from a developer machine uses the OAuth session from `wrangler login` and does **not** need either. Revisit when Phase 6 is picked up.~~ _(Resolved 2026-05-26: Phase 6 was un-deferred and the API token + GitHub secrets were created alongside it. See Phase 6 line 137.)_

- [x] ~~_(Deferred)_~~ User creates a scoped API token at https://dash.cloudflare.com/profile/api-tokens → "Create Custom Token":
  ...
  - Copy the token immediately — it's shown once.
    _(Completed 2026-05-26 under Phase 6 — see line 137.)_
- [x] ~~_(Deferred)_~~ User finds the Account ID (right sidebar of the Cloudflare dashboard, any account-scoped page). _(Completed 2026-05-26 under Phase 6: `016342b5c7cf5429f151e7773cf26c44`.)_
- [x] ~~_(Deferred)_~~ Add to GitHub repo secrets (Settings → Secrets and variables → Actions):
...
- [x] Verify with `npx wrangler whoami` — should show the account email and ID. _(Verified 2026-05-25: konporeba@gmail.com, account `016342b5c7cf5429f151e7773cf26c44`.)_
...
**Done when:** `wrangler whoami` returns the right account, GitHub secrets populated. _(Phase 3 active scope complete 2026-05-25: local OAuth verified — single account `Konporeba@gmail.com's Account` / `016342b5c7cf5429f151e7773cf26c44`, so the multi-account `account_id` pin is unnecessary; token shows full Workers + Secrets Store scopes needed for Phases 4–5. GitHub-secrets sub-task travels with Phase 6, deferred.)_
...
- [x] **User runs** (not the agent — these prompt for the value and write directly to prod):
  ...
  ```
  Each command prompts for the secret value; values are stored in Workers Secrets Store, not the dashboard env tab. _(Done 2026-05-25 by user; placeholder Worker `mind-tutor` created on first `secret put` as expected.)_
- [x] **Do not** add `SUPABASE_URL` or `SUPABASE_KEY` to the `vars` block in `wrangler.jsonc` — that block is plaintext and committed to git. _(Verified: `wrangler.jsonc` has no `vars` block; secrets stay in Secrets Store only.)_
- [x] Verify with `npx wrangler secret list` — should show both names (values are not shown). _(Verified 2026-05-25: both `SUPABASE_URL` and `SUPABASE_KEY` present as `secret_text`.)_
...
**Done when:** `wrangler secret list` shows both secrets. _(Phase 4 complete 2026-05-25.)_
...
- [x] Agent runs `npm run build` and confirms `dist/_worker.js/index.js` and `dist/` static assets exist. _(Done 2026-05-26: produced `dist/server/entry.mjs` + `dist/server/wrangler.json` + `dist/client/_` per the Phase 1 layout-divergence note; build green in 48.61s.)\*
- [x] Agent runs `npx wrangler deploy --dry-run --outdir=dist-dryrun` to validate the bundle without uploading (this writes to a temp dir and reports the bundle size — useful sanity check). Delete `dist-dryrun/` afterward. _(Done 2026-05-26: bundle 1910.70 KiB / 390.68 KiB gzip, 21 modules, bindings `SESSION`/`IMAGES`/`ASSETS` resolved. Heaviest chunks: `worker-entry` 803 KiB, `supabase` 706 KiB. `dist-dryrun/` deleted.)_
- [x] **User runs** `npx wrangler deploy`. Output shows the deployed URL: `https://mind-tutor.<account-subdomain>.workers.dev`. _(Done 2026-05-26: `Deployed mind-tutor triggers (5.56 sec)` → `https://mind-tutor.konporeba.workers.dev`, version ID `e2c8062c-3301-4e10-ab22-792bb16968fe`.)_
- [x] User opens the URL in a browser; verify: _(All confirmed 2026-05-26 by user: signed in successfully on the live URL. `/auth/signin` form renders per agent-side fetch; sign-in round-trip and `/dashboard` access both worked in the browser.)_
...
- [x] Phase 3 deferred items: user created scoped Cloudflare API token (`Workers Scripts:Edit`, `Account Settings:Read`, `User Memberships:Read`, account-scoped to `Konporeba@gmail.com's Account`) and added `CLOUDFLARE_API_TOKEN` + `CLOUDFLARE_ACCOUNT_ID` (`016342b5c7cf5429f151e7773cf26c44`) to GitHub repo secrets. _(Done 2026-05-26.)_
- [x] Edit `.github/workflows/ci.yml` (or split into a new `deploy.yml` — keeping it in one file is fine for a single deploy target):
  ...
  - The action does **not** automatically run `astro build` — the build step must run before the action, and the action just invokes `wrangler deploy`.
    _(Done 2026-05-26. Build step also runs `npx astro sync` before `npm run build` to match the existing `ci` job's astro:env type generation; same `SUPABASE_URL`/`SUPABASE_KEY` repo secrets feed both build steps.)_
- [x] Pin the action by tag (`@v3`) and document in the workflow that bumping it requires re-reading the changelog (infra plan risk: `@astrojs/cloudflare` and `wrangler-action` both iterate fast). _(Done 2026-05-26 — pinned `cloudflare/wrangler-action@v3` with an inline comment in the workflow.)_
- [x] **Verify** by pushing a no-op commit to a branch, opening a PR, merging, watching the deploy job in the Actions tab. _(Done 2026-05-26 — see Phase 9 follow-up for the live-run details; verification was done via direct push to `master` rather than PR + merge since the repo's single-author flow doesn't gate on PRs.)_
...
- [x] Use the `update-config` skill (or edit `.claude/settings.local.json` by hand) to add two MCP servers:
  ...
  ```
  _(Done 2026-05-26. **Plan-path correction:** Claude Code's settings.json schema has no `mcpServers` property, so the literal write into `.claude/settings.local.json` would be silently ignored. The canonical Claude Code MCP entrypoint is `.mcp.json` at the project root, gated by `enabledMcpjsonServers` in settings. Final layout:_
  ...
  - `.claude/settings.local.json` (now gitignored; new line added to `.gitignore`) — holds `{"enabledMcpjsonServers": ["cloudflare-observability", "cloudflare-bindings"]}` so the harness auto-approves them on load.
    _Verified with `claude mcp list`: both servers are discovered (status `✗ Failed to connect` is the expected pre-OAuth state — the handshake fires on first tool call after Claude Code reload, per the next sub-task).)_
- [x] Restart Claude Code; first tool call to either server triggers a browser OAuth handshake — complete it once per machine. _(Done 2026-05-26: both servers loaded in this session and tool calls succeeded without OAuth prompts — handshake was completed in the prior session.)_
- [x] Verify by asking the agent to "list recent logs for mind-tutor" (Observability) and "list bindings on mind-tutor" (Bindings). _(Verified 2026-05-26 with `set_active_account` → `016342b5c7cf5429f151e7773cf26c44` on both servers. Observability returned 5 most-recent `mind-tutor` fetch events from the last 24h (all `outcome: ok`, all version `e2c8062c-3301-4e10-ab22-792bb16968fe`, CPU 2–7 ms, wall 3–137 ms) covering `/`, `/dashboard`, `/auth/signin`, `/auth/signup`, `POST /api/auth/signout`. Bindings returned the Worker record: `name: mind-tutor`, id `9f4eff4795d44c20a52ad654419a49e6`.)_
...
**Done when:** both MCP servers respond to a structured query about the deployed Worker. _(Phase 7 complete 2026-05-26.)_
...
- [x] Create `X:\MindTutor\context\deployment\` directory. _(Already existed — `deployment-plan.md` lives here.)_
- [x] Write `deploy-plan.md` covering: _(Done 2026-05-26 — file at `context/deployment/deploy-plan.md`. Covers all listed sections plus a Quick-reference command table and a "What's not deployed" carry-forward list.)_
...
- [x] Stage and commit the changes from Phases 1, 2, 6, 8: `wrangler.jsonc` (rename), `package.json` (engines field), `.dev.vars.example`, `.github/workflows/ci.yml` (deploy job), `context/deployment/deploy-plan.md`. _(Done 2026-05-26 as commit `d897a01` "Deploy mind-tutor to Cloudflare Workers, wire Cloudflare MCP". Phase 6's `.github/workflows/ci.yml` deploy-job edit was skipped since Phase 6 is deferred; commit also picked up `.gitignore` (settings.local.json exclusion) and `.mcp.json` (the MCP server defs, since the plan's literal `.claude/settings.local.json` location wouldn't be read by Claude Code — see Phase 7 correction note).)_
- [x] Push to `master`; watch the CI deploy job complete; verify the live URL still serves the authenticated dashboard. _(Pushed 2026-05-26: `2911647..d897a01 master -> master`. CI auto-deploy job is deferred with Phase 6 so there's no auto-deploy run to watch; existing CI build workflow runs on push but doesn't deploy. Live URL still serves the authenticated dashboard — re-verified during the tail step below.)_ **Phase 6 follow-up 2026-05-26:** after Phase 6 was un-deferred and the deploy job landed in commit `f8d61dc`, the resulting CI run (`26465566928`) completed end-to-end in ~1m40s (`ci` 49s + `deploy` 50s, well under the 2-min target). The action shipped Worker version `503c0cfd-9859-4cbd-93bb-8b819c6ea876` (`wrangler deployments list --name mind-tutor` confirms, replacing the prior manual deploy `e2c8062c…`). Smoke-tested the live URL: `/` → 200, `/auth/signin` → 200, `/dashboard` → 302 (unauthenticated redirect), all sub-350 ms.
- [x] Stream logs with `npx wrangler tail mind-tutor` for 60 seconds while clicking through the dashboard to confirm no `Worker exceeded CPU time limit` warnings. _(Done 2026-05-26: captured 8 requests in ~60s — full auth cycle (`/auth/signin` → POST signin → `/` → `/dashboard` → POST signout → `/` → `/auth/signin` → `/auth/signup`). Every line reported status `Ok`. Zero CPU-limit warnings, zero 5xx, zero errors. Free-tier CPU budget comfortably surviving the normal session shape.)_

**Done when:** the merge auto-deploy is green and the live URL behaves identically to local dev. _(Phase 9 fully met 2026-05-26 once Phase 6 was un-deferred and the first auto-deploy on commit `f8d61dc` shipped Worker version `503c0cfd…` green — see Phase 9 follow-up bullet above.)_

---

## Verification checklist (end-to-end)

- [x] `npx wrangler whoami` returns the correct account. _(Phase 3, 2026-05-25.)_
- [x] `npx wrangler secret list` shows `SUPABASE_URL` and `SUPABASE_KEY`. _(Phase 4, 2026-05-25.)_
- [x] `https://mind-tutor.<account>.workers.dev/` loads. _(Phase 5, 2026-05-26 → `https://mind-tutor.konporeba.workers.dev`.)_
- [x] `/auth/signup` → `/auth/signin` → `/dashboard` flow works on the live URL. _(Phase 5, 2026-05-26.)_
- [x] `/dashboard` redirects unauthenticated requests to `/auth/signin`. _(Phase 5, 2026-05-26.)_
- [x] CI deploy job runs on merge to `master` and finishes in <2 min. _(Phase 6, 2026-05-26 — run `26465566928` finished in ~1m40s, shipped version `503c0cfd…`.)_
- [x] `npx wrangler tail mind-tutor` shows no CPU-limit warnings during a normal session. _(Phase 9, 2026-05-26; re-confirmed via Observability MCP 2026-05-26 — last 24h CPU 2–7 ms, no `Worker exceeded CPU time limit` events.)_
- [x] Both Cloudflare MCP servers respond to structured queries. _(Phase 7, 2026-05-26.)_
