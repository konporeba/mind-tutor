...
Cloudflare Workers and clears all four agent-friendly gates. Bootstrapper
...
on Cloudflare Workers (Workers Static Assets) — the canonical 2026 path for
SSR Astro on Cloudflare (Pages is soft-deprecated for new projects), and the
cheapest free-tier path for a medium-scale MVP. Canonical deploy command is
`wrangler deploy`, not `wrangler pages deploy`. GitHub Actions with auto-deploy-on-merge is the
